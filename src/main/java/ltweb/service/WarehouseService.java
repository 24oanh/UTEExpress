...
public class WarehouseService {
	...
	private void updateInventoryOnInbound(Long warehouseId, Long packageId, Integer quantity) {
		Inventory inventory = inventoryRepository.findByWarehouseIdAndPackageItemId(warehouseId, packageId)
				.orElse(Inventory.builder().warehouse(warehouseRepository.findById(warehouseId).orElseThrow())
						.packageItem(packageRepository.findById(packageId).orElseThrow()).quantity(0)
						.deliveredQuantity(0).remainingQuantity(0).build());
