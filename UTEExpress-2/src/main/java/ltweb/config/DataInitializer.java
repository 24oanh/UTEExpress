...
public class DataInitializer implements CommandLineRunner {
	...
	public void run(String... args) {
		...
		Role warehouseRole = roleRepository.findByName(RoleType.ROLE_WAREHOUSE_STAFF).orElseThrow();
		Role shipperRole = roleRepository.findByName(RoleType.ROLE_SHIPPER).orElseThrow();
		Role customerRole = roleRepository.findByName(RoleType.ROLE_CUSTOMER).orElseThrow();
		User adminUser = User.builder()
			    .username("admin")  // Username là "admin"
			    .password(passwordEncoder.encode("Admin@123"))
			    .email("admin@uteexpress.com")
			    .fullName("System Admin")
			    .phone("0900000000")
			    .isActive(true)  // Đảm bảo isActive = true
		...
		User customerUser = User.builder()
			    ...
			    .phone("0912345678")
			    .isActive(true)  // ✅ Đảm bảo true
			    .roles(Set.of(customerRole))
			    .build();
			customerUser = userRepository.save(customerUser);

			// ✅ THÊM: Tạo Customer entity
	...
	private void initRoles() {
	    try {
	        if (roleRepository.findByName(RoleType.ROLE_WAREHOUSE_STAFF).isEmpty()) {
	            Role role = new Role();
	            role.setName(RoleType.ROLE_WAREHOUSE_STAFF);
	            roleRepository.save(role);
	        }
	    } catch (Exception e) {
	        System.err.println("Error creating WAREHOUSE_STAFF role: " + e.getMessage());
	    }
	    
	    try {
	        if (roleRepository.findByName(RoleType.ROLE_SHIPPER).isEmpty()) {
	            Role role = new Role();
	            role.setName(RoleType.ROLE_SHIPPER);
	            roleRepository.save(role);
	        }
	    } catch (Exception e) {
	        System.err.println("Error creating SHIPPER role: " + e.getMessage());
	    }
	    ...
	    try {
	        ...
	        System.err.println("Error creating CUSTOMER role: " + e.getMessage());
	    }
	    
	    try {
	        if (roleRepository.findByName(RoleType.ROLE_SUPPORT).isEmpty()) {
	            Role role = new Role();
	            role.setName(RoleType.ROLE_SUPPORT);
	            roleRepository.save(role);
	        }
	    } catch (Exception e) {
	        System.err.println("Error creating SUPPORT role: " + e.getMessage());
	    }
